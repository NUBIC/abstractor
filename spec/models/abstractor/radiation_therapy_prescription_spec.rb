...
describe RadiationTherapyPrescription do
  ...
  describe "abstracting" do
    ...
    it "creates a 'has_anatomical_location' abstraction'" do
      ...
      @radiation_therapy_prescription.reload.detect_abstractor_abstraction(@abstractor_abstraction_schema_has_anatomical_location).should_not be_nil
    end
    ...
    it "creates a 'has_laterality' abstraction'" do
      @radiation_therapy_prescription.abstract
      @radiation_therapy_prescription.reload.detect_abstractor_abstraction(@abstractor_abstraction_schema_has_laterality).should_not be_nil
    end
    ...
    it "creates a 'has_anatomical_location' abstraction suggestion suggested value from an abstractor object value" do
      ...
      radiation_therapy_prescription.abstract
      radiation_therapy_prescription.reload.detect_abstractor_abstraction(@abstractor_abstraction_schema_has_anatomical_location).abstractor_suggestions.first.suggested_value.should == 'parietal lobe'
    end
    ...
    it "creates a 'has_anatomical_location' abstraction suggestion match value from a from an abstractor object value" do
      radiation_therapy_prescription = FactoryGirl.create(:radiation_therapy_prescription, site_name: 'left parietal lobe')
      radiation_therapy_prescription.abstract
      radiation_therapy_prescription.reload.detect_abstractor_abstraction(@abstractor_abstraction_schema_has_anatomical_location).abstractor_suggestions.first.abstractor_suggestion_sources.first.match_value.should == 'left parietal lobe'
    end
    ...
    it "creates a 'has_anatomical_location' abstraction suggestion match value from a from an abstractor object value variant" do
      radiation_therapy_prescription = FactoryGirl.create(:radiation_therapy_prescription, site_name: 'left parietal')
      radiation_therapy_prescription.abstract
      radiation_therapy_prescription.reload.detect_abstractor_abstraction(@abstractor_abstraction_schema_has_anatomical_location).abstractor_suggestions.first.abstractor_suggestion_sources.first.match_value.should == 'left parietal'
    end
    ...
    it "creates multiple 'has_anatomical_location' abstraction suggestion match values given multiple different matches" do
      ...
      radiation_therapy_prescription.abstract
      Set.new(radiation_therapy_prescription.reload.detect_abstractor_abstraction(@abstractor_abstraction_schema_has_anatomical_location).abstractor_suggestions.first.abstractor_suggestion_sources.map(&:match_value)).should  == Set.new(["left parietal lobe.", "let me remind you that it is the left parietal."])
    end
    ...
    it "does not create another 'has_anatomical_location' abstraction suggestion upon re-abstraction" do
      radiation_therapy_prescription = FactoryGirl.create(:radiation_therapy_prescription, site_name: 'left parietal lobe')
      radiation_therapy_prescription.abstract
      radiation_therapy_prescription.reload.detect_abstractor_abstraction(@abstractor_abstraction_schema_has_anatomical_location).abstractor_suggestions.size.should == 1
      radiation_therapy_prescription.abstract
      radiation_therapy_prescription.reload.detect_abstractor_abstraction(@abstractor_abstraction_schema_has_anatomical_location).abstractor_suggestions.size.should == 1
    end
    ...
    it "creates multiple 'has_anatomical_location' abstraction suggestions given multiple different matches" do
      ...
      Set.new(radiation_therapy_prescription.reload.detect_abstractor_abstraction(@abstractor_abstraction_schema_has_anatomical_location).abstractor_suggestions.map(&:suggested_value)).should == Set.new(['cerebral meninges', 'parietal lobe', 'meninges'])
    end
    ...
    it "creates one 'has_anatomical_location' abstraction suggestion given multiple identical matches" do
      ...
      radiation_therapy_prescription.abstract
      radiation_therapy_prescription.reload.detect_abstractor_abstraction(@abstractor_abstraction_schema_has_anatomical_location).abstractor_suggestions.select { |abstractor_suggestion| abstractor_suggestion.suggested_value == 'parietal lobe'}.size.should == 1
    end
    ...
    it "does not create a 'has_anatomical_location' abstraction suggestion match value from a negated value" do
      ...
      radiation_therapy_prescription.abstract
      radiation_therapy_prescription.reload.detect_abstractor_abstraction(@abstractor_abstraction_schema_has_anatomical_location).abstractor_suggestions.first.abstractor_suggestion_sources.first.match_value.should be_nil
    end
    ...
    it "creates one 'has_anatomical_location' abstraction suggestion source given multiple identical matches" do
      ...
      radiation_therapy_prescription.abstract
      radiation_therapy_prescription.reload.detect_abstractor_abstraction(@abstractor_abstraction_schema_has_anatomical_location).abstractor_suggestions.select { |abstractor_suggestion| abstractor_suggestion.abstractor_suggestion_sources.first.match_value == 'left parietal lobe.'}.size.should == 1
    end
    ...
    it "does not create another 'has_anatomical_location' abstraction suggestion source upon re-abstraction (using the canonical name/value format)" do
      radiation_therapy_prescription = FactoryGirl.create(:radiation_therapy_prescription, site_name: 'left parietal lobe')
      radiation_therapy_prescription.abstract
      radiation_therapy_prescription.reload.detect_abstractor_abstraction(@abstractor_abstraction_schema_has_anatomical_location).abstractor_suggestions.first.abstractor_suggestion_sources.size.should == 1
      radiation_therapy_prescription.abstract
      radiation_therapy_prescription.reload.detect_abstractor_abstraction(@abstractor_abstraction_schema_has_anatomical_location).abstractor_suggestions.first.abstractor_suggestion_sources.size.should == 1
    end
    ...
    it "creates a 'has_anatomical_location' abstraction suggestion object value for each suggestion with a suggested value" do
      ...
      radiation_therapy_prescription.reload.detect_abstractor_abstraction(@abstractor_abstraction_schema_has_anatomical_location).abstractor_suggestions.first.abstractor_object_value.should == abstractor_object_value
    end
    ...
    it "creates a 'has_anatomical_location' unknown abstraction suggestion" do
      ...
      radiation_therapy_prescription.abstract
      radiation_therapy_prescription.reload.detect_abstractor_abstraction(@abstractor_abstraction_schema_has_anatomical_location).abstractor_suggestions.first.unknown.should be_true
    end

    it "does not create a 'has_anatomical_location' abstraction suggestion object value for a unknown abstraction suggestion " do
      radiation_therapy_prescription = FactoryGirl.create(:radiation_therapy_prescription, site_name: 'Forgot to mention an anatomical location.')
      radiation_therapy_prescription.abstract
      radiation_therapy_prescription.reload.detect_abstractor_abstraction(@abstractor_abstraction_schema_has_anatomical_location).abstractor_suggestions.first.abstractor_object_value.should be_nil
    end

    it "does not creates another 'has_anatomical_location' unknown abstraction suggestion upon re-abstraction" do
      radiation_therapy_prescription = FactoryGirl.create(:radiation_therapy_prescription, site_name: 'Forgot to mention an anatomical location.')
      radiation_therapy_prescription.abstract
      radiation_therapy_prescription.reload.detect_abstractor_abstraction(@abstractor_abstraction_schema_has_anatomical_location).abstractor_suggestions.select { |abstractor_suggestion| abstractor_suggestion.unknown }.size.should == 1
      radiation_therapy_prescription.abstract
      radiation_therapy_prescription.reload.detect_abstractor_abstraction(@abstractor_abstraction_schema_has_anatomical_location).abstractor_suggestions.select { |abstractor_suggestion| abstractor_suggestion.unknown }.size.should == 1
    end
