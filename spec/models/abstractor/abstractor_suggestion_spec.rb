...
describe  Abstractor::AbstractorSuggestion do
  ...
  it "can detect a suggestion from a suggested value", focus: false do
    ...
    abstractor_abstraction_source = FactoryGirl.create(:abstractor_abstraction_source, abstractor_subject: abstractor_abstraction_schema.abstractor_subjects.first, abstractor_rule_type: abstractor_rule_type, abstractor_abstraction_source_type: abstractor_abstraction_source_type)
    ...
    expect(abstractor_suggestion_bar.detect_abstractor_suggestion_source(abstractor_abstraction_source, 'bar', 1, 'Foo', 'boo')).to eq(abstractor_suggestion_source_bar)
  end
end
