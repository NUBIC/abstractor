...
describe  Abstractor::AbstractorAbstractionSource do
  ...
  it "can normalize its from method from a string", focus: false do
    ...
    expect(abstractor_abstraction_source.normalize_from_method_to_sources(@encounter_note)).to eq([ { source_type: EncounterNote, source_id: @encounter_note.id, source_method: "note_text"}])
  end
  ...
  it "can normalize its from method when the method returns nil", focus: false do
    ...
    expect(abstractor_abstraction_source.normalize_from_method_to_sources(encounter_note)).to eq([{ source_type: EncounterNote, source_id: encounter_note.id, source_method: "custom_method_nil" }])
  end
  ...
  it "can normalize its from method when it is nil", focus: false do
    ...
    expect(abstractor_abstraction_source.normalize_from_method_to_sources(encounter_note)).to eq([{ source_type: EncounterNote, source_id: encounter_note.id, source_method: nil }])
  end
