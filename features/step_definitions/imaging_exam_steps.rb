Given /^imaging exams with the following information exist$/ do |table|
  table.hashes.each_with_index do |imaging_exam_hash, i|
    imaging_exam = FactoryGirl.create(:imaging_exam, note_text: imaging_exam_hash['Note Text'])
    imaging_exam.abstract
    if imaging_exam_hash['Status'] && imaging_exam_hash['Status'] == 'Reviewed'
      abstractor_suggestion_status_accepted= AbstractorSuggestionStatus.where(:name => 'Accepted').first
      imaging_exam.reload.abstractor_abstractions(true).each do |abstractor_abstraction|
